from pymongo.errors import DuplicateKeyError
from app.apis.database import database as db
from bson.objectid import ObjectId
from .models import ContentItem, ContentCategory
from typing import Optional
import logging

logger = logging.getLogger(__name__)

content_collection = db.get_collection("content_items")
content_collection.create_index("url", unique=True)

def insert_content_item(data: dict) -> Optional[ContentItem]:
  """
  Insert a new content item to the database.
  Return the ContentItem object if the operation is succeed.
  """
  logger.info('add_content')
  try:
    item = content_collection.insert_one(data)
    new_item = content_collection.find_one({"_id": item.inserted_id})
    return _to_content_item(new_item)
  except DuplicateKeyError:
    logger.warning("A content item with the same URL already exists.")
    return None  

def update_content_item(id: str, data: dict) -> bool:
  """
  Update the content item with the specified url or id.
  Return True if the update is succeed, False otherwise.
  """
  item = content_collection.find_one({"_id": ObjectId(id)})
  if item:
    updated_item = content_collection.update_one(
      {"_id": ObjectId(id)},
      {"$set": data}
    )
    return updated_item.matched_count > 0
  return False

def retrieve_content_item(url: str, id: Optional[str]=None) -> Optional[ContentItem]:
  """
  Return the content item with the specified url or id.
  """
  if id:
    query = {"_id": ObjectId(id)}
  elif url:
    query = {"url": url}
  result = content_collection.find_one(query)
  if result:
    return _to_content_item(result)
  return None

def retrieve_all_content_items():
  """
  Return all the content items in the database.
  """
  items = []
  cursor = content_collection.find()
  for item in cursor.sort({"_id": -1}): # sort by time, descending
    items.append(_to_content_item(item))
  return items

def retrieve_content_items_by_category(category_id: str):
  """
  Return all the content items with the specified category_id in the database.
  """
  items = []
  cursor = content_collection.find({"category.id": category_id})
  for item in cursor.sort({"_id": -1}): # sort by time, descending
    items.append(_to_content_item(item))
  return items

def retrieve_categories():
  """
  Return all the categories existing in the knowledge base.
  """
  pipeline = [
    {"$project": {"category_name": "$category.name"}},
    {"$group": {"_id": "$category_name"}}
  ]
  unique_categories = list(content_collection.aggregate(pipeline))
  categories = []
  for category in unique_categories:
    if category["_id"] is not None:
      categories.append(category["_id"])
  return categories

def _to_content_item(data: dict) -> ContentItem:
  return ContentItem(
    id=str(data["_id"]),
    url=data["url"],
    title=data["title"],
    description=data["description"],
    thumbnail=data["thumbnail"],
    full_text=data.get("full_text", None),
    ai_labels=data.get('ai_labels', None),
    ai_highlights=data.get('ai_highlights', None),
    ai_summary=data.get('ai_summary', None),
    ai_podcast_url=data.get('ai_podcast_url', None),
    category=_to_category(data.get("category", None))
  )

def _to_category(data: dict) -> ContentCategory:
  if data == None:
    return None
  return ContentCategory(
    id=data["id"],
    name=data["name"],
    description=data["description"]
  )